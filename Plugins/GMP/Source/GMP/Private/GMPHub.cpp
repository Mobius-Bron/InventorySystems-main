...
namespace GMP
{
	...
	void FMessageHub::AddRoute(const FName& BehaviorKey, const TArray<FName>& OpKeys)
	{
		if (BehaviorKey.IsNone())
		{
			return;
		}
		// Store/replace the raw declared ops. Replacing (or adding) a behavior may change how other behavior
		// keys expand, so invalidate the whole lazy flatten cache; it rebuilds on next resolve. AddRoute is
		// startup-time only, so a full reset here is cheap.
		RouteTable.Add(BehaviorKey, OpKeys);
		FlattenedRouteCache.Reset();
	}

	void FMessageHub::FlattenRouteImpl(const FName& Key, TSet<FName> Visiting, TArray<FName>& OutFlattened) const
	{
		if (Key.IsNone())
		{
			return;
		}
		// Cycle guard: if Key is already on the current expansion path, stop (and warn) to avoid infinite recursion.
		if (Visiting.Contains(Key))
		{
			GMP_WARNING(TEXT("[GMP.Route] cycle detected at behavior key '%s'; skipping its expansion."), *Key.ToString());
			return;
		}
		Visiting.Add(Key);

		if (const TArray<FName>* Ops = RouteTable.Find(Key))
		{
			// Key is itself a behavior: recurse into its raw ops, carrying the updated visiting set (by value).
			for (const FName& Op : *Ops)
			{
				FlattenRouteImpl(Op, Visiting, OutFlattened);
			}
		}
		else
		{
			// Leaf op key (not a behavior). Dedup so a shared sub-op is delivered once.
			OutFlattened.AddUnique(Key);
		}
	}
