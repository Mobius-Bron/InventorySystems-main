...
// EAutomationTestFlags mask was flattened from EAutomationTestFlags::ApplicationContextMask to EAutomationTestFlags_ApplicationContextMask (UE5.5 already ships the flattened form).
...
// A behavior key fans out to op keys; the hot path expands the route directly (no forwarder runs). Covers
// param forwarding, chained routes (nested behavior keys), and zero impact on plain (non-route) sends.
static bool Test_RouteFlattening()
{
	GMP_TEST_BEGIN("Route.flattening (fan-out, params, chained)");
	UObject* Listener = MakeProbe();

	int32 GotA = 0, GotB = 0, GotC = 0, GotD = 0;
	FSigHandle HA, HB, HC, HD;
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.OpA"), Listener, &HA, [&](int32 V) { GotA += V; });
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.OpB"), Listener, &HB, [&](int32 V) { GotB += V; });
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.OpC"), Listener, &HC, [&](int32 V) { GotC += V; });
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.OpD"), Listener, &HD, [&](int32 V) { GotD += V; });

	// flat route: behavior -> 3 ops; the int arg is forwarded to every op
	Hub()->AddRoute(FName("GMP.UT.Route.Behavior"), { FName("GMP.UT.Route.OpA"), FName("GMP.UT.Route.OpB"), FName("GMP.UT.Route.OpC") });
	Hub()->SendObjectMessage(MSGKEY("GMP.UT.Route.Behavior"), Listener, int32(2));
	GMP_TEST_CHECK(GotA == 2);
	GMP_TEST_CHECK(GotB == 2);
	GMP_TEST_CHECK(GotC == 2);
	GMP_TEST_CHECK(GotD == 0);  // OpD not routed

	// chained route: BehaviorAll -> [Behavior(flat), OpD]; Behavior expands once, OpD once (no dup)
	Hub()->AddRoute(FName("GMP.UT.Route.BehaviorAll"), { FName("GMP.UT.Route.Behavior"), FName("GMP.UT.Route.OpD") });
	Hub()->SendObjectMessage(MSGKEY("GMP.UT.Route.BehaviorAll"), Listener, int32(3));
	GMP_TEST_CHECK(GotA == 5);
	GMP_TEST_CHECK(GotB == 5);
	GMP_TEST_CHECK(GotC == 5);
	GMP_TEST_CHECK(GotD == 3);

	// non-route key still works exactly as before (zero impact when a route isn't registered for it)
	int32 GotPlain = 0;
	FSigHandle HPlain;
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.Plain"), Listener, &HPlain, [&](int32 V) { GotPlain += V; });
	Hub()->SendObjectMessage(MSGKEY("GMP.UT.Route.Plain"), Listener, int32(9));
	GMP_TEST_CHECK(GotPlain == 9);

	// removing a route restores normal semantics (behavior key no longer fans out)
	Hub()->RemoveRoute(FName("GMP.UT.Route.Behavior"));
	Hub()->RemoveRoute(FName("GMP.UT.Route.BehaviorAll"));

	Listener->RemoveFromRoot();
	GMP_TEST_END();
}
GMP_IMPLEMENT_AUTOMATION_TEST(Test_RouteFlattening, "GMP.Route.Flattening")

// ---- Route cycle detection -------------------------------------------------
// A -> B -> A must terminate (cycle broken) while still delivering to the reachable leaf op.
static bool Test_RouteCycle()
{
	GMP_TEST_BEGIN("Route.cycle detection (terminates)");
	UObject* Listener = MakeProbe();

	int32 Got = 0;
	FSigHandle H;
	Hub()->ListenObjectMessage(MSGKEY("GMP.UT.Route.CycleOp"), Listener, &H, [&](int32 V) { Got += V; });

	Hub()->AddRoute(FName("GMP.UT.Route.CycleA"), { FName("GMP.UT.Route.CycleB") });
	Hub()->AddRoute(FName("GMP.UT.Route.CycleB"), { FName("GMP.UT.Route.CycleA"), FName("GMP.UT.Route.CycleOp") });

	Hub()->SendObjectMessage(MSGKEY("GMP.UT.Route.CycleA"), Listener, int32(1));
	GMP_TEST_CHECK(Got == 1);  // leaf reached exactly once, recursion terminated

	Hub()->RemoveRoute(FName("GMP.UT.Route.CycleA"));
	Hub()->RemoveRoute(FName("GMP.UT.Route.CycleB"));

	Listener->RemoveFromRoot();
	GMP_TEST_END();
}
GMP_IMPLEMENT_AUTOMATION_TEST(Test_RouteCycle, "GMP.Route.Cycle")
...
int32 RunAllGMPTests(const FString& Params)
{
	...
	// declarative route flattening (gate-independent)
	Test_RouteFlattening();
	Test_RouteCycle();
#if GMP_WITH_DIRECT_SIGNAL
	if (!bNoDirect)
	{
