...
// AddRoute(behaviorkey, opkeys_table) -- declare a behavior key that fans out to op keys. Sending the behavior
// key afterwards (locally or over the network) is flattened into direct sends to the op keys.
...
inline int Lua_AddRoute(lua_State* L)
{
	do
	{
		...
		if (!ensure(NumArgs >= 2))
			break;
		const FName BehaviorKey = UTF8_TO_TCHAR(luaL_checkstring(L, 1));
		...
		{
			lua_rawgeti(L, 2, i);
			const FName Op = UTF8_TO_TCHAR(luaL_checkstring(L, -1));
			if (!Op.IsNone())
			{
				OpKeys.Add(Op);
			}
...
inline void GMP_RegisterToSlua(lua_State* L)
{
	...
	LuaObject::addGlobalMethod(L, "AddRoute", Lua_AddRoute);
#if defined(GMP_SLUA_STATIC_BIND) && GMP_SLUA_STATIC_BIND
