...
UOldMIS_EquipmentComponent::UOldMIS_EquipmentComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
	SetIsReplicatedByDefault(true);
}

void UOldMIS_EquipmentComponent::SetOwningSkeletalMesh(USkeletalMeshComponent* OwningMesh)
{
...
void UOldMIS_EquipmentComponent::Init(APlayerController* InPC, UOldMIS_InventoryComponent* InInvComp, USkeletalMeshComponent* InMesh)
{
...
AOldMIS_EquipActor* UOldMIS_EquipmentComponent::SpawnEquippedActor(FOldMIS_EquipmentFragment* EquipmentFragment, const FOldMIS_ItemManifest& Manifest, USkeletalMeshComponent* AttachMesh)
{
	DH_PRINT(EDH_Output::Both, 4.f, DHColors::Cyan,
	...
	AOldMIS_EquipActor* SpawnedEquipActor = EquipmentFragment->SpawnAttachedActor(AttachMesh);
...
AOldMIS_EquipActor* UOldMIS_EquipmentComponent::FindEquippedActor(const FGameplayTag& EquipmentTypeTag)
{
	auto FoundActor = EquippedActors.FindByPredicate([&EquipmentTypeTag](const AOldMIS_EquipActor* EquippedActor)
	{
...
void UOldMIS_EquipmentComponent::RemoveEquippedActor(const FGameplayTag& EquipmentTypeTag)
{
	if (AOldMIS_EquipActor* EquippedActor = FindEquippedActor(EquipmentTypeTag); IsValid(EquippedActor))
	{
...
void UOldMIS_EquipmentComponent::OnItemEquipped(UOldMIS_InventoryItem* EquippedItem)
{
	DH_PRINT(EDH_Output::Both, 4.f, DHColors::Green,
	...
	FOldMIS_ItemManifest& ItemManifest = EquippedItem->GetItemManifestMutable();
	FOldMIS_EquipmentFragment* EquipmentFragment = ItemManifest.GetFragmentOfTypeMutable<FOldMIS_EquipmentFragment>();
	...
	AOldMIS_EquipActor* SpawnedEquipActor = SpawnEquippedActor(EquipmentFragment, ItemManifest, OwningSkeletalMesh.Get());
...
void UOldMIS_EquipmentComponent::OnItemUnequipped(UOldMIS_InventoryItem* UnequippedItem)
{
	if (!IsValid(UnequippedItem)) return;
	if (!OwningPlayerController->HasAuthority()) return;

	FOldMIS_ItemManifest& ItemManifest = UnequippedItem->GetItemManifestMutable();
	FOldMIS_EquipmentFragment* EquipmentFragment = ItemManifest.GetFragmentOfTypeMutable<FOldMIS_EquipmentFragment>();
