...
#include "Interaction/OldMIS_Highlightable.h"
...
UOldMIS_InventoryComponent::UOldMIS_InventoryComponent() : InventoryList(this)
{
...
void UOldMIS_InventoryComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ThisClass, InventoryList);
}

void UOldMIS_InventoryComponent::Init(APlayerController* InPC)
{
...
void UOldMIS_InventoryComponent::TraceForItem()
{
	...
	if (ThisActor.IsValid())
	{
		if (UActorComponent* Highlightable = ThisActor->FindComponentByInterface(UOldMIS_Highlightable::StaticClass()); IsValid(Highlightable))
		{
			IOldMIS_Highlightable::Execute_Highlight(Highlightable);
		}

		if (UOldMIS_ItemComponent* ItemComponent = ThisActor->FindComponentByClass<UOldMIS_ItemComponent>())
		{
	...
	{
		if (UActorComponent* Highlightable = LastActor->FindComponentByInterface(UOldMIS_Highlightable::StaticClass()); IsValid(Highlightable))
		{
			IOldMIS_Highlightable::Execute_UnHighlight(Highlightable);
		}
	}
}

void UOldMIS_InventoryComponent::PrimaryInteract()
{
	...
	UOldMIS_ItemComponent* ItemComp = ThisActor->FindComponentByClass<UOldMIS_ItemComponent>();
	if (!IsValid(ItemComp))
	{
...
void UOldMIS_InventoryComponent::ToggleInventory()
{
...
void UOldMIS_InventoryComponent::TryAddItem(UOldMIS_ItemComponent* ItemComponent)
{
	...
	FOldMIS_SlotAvailabilityResult Result = InventoryWidget->HasRoomForItem(ItemComponent);

	DH_SCREEN(3.f, DHColors::Cyan, "[背包组件] HasRoomForItem | 总空间=%d | 槽位数=%d", Result.TotalRoomToFill, Result.SlotAvailabilities.Num());

	UOldMIS_InventoryItem* FoundItem = InventoryList.FindFirstItemByType(ItemComponent->GetItemManifest().GetItemType());
...
void UOldMIS_InventoryComponent::Server_AddNewItem_Implementation(AActor* ItemActor, int32 StackCount, int32 Remainder)
{
	...
	UOldMIS_ItemComponent* ItemComponent = ItemActor->FindComponentByClass<UOldMIS_ItemComponent>();
	...
	UOldMIS_InventoryItem* NewItem = InventoryList.AddEntry(ItemComponent);
	...
	else if (FOldMIS_StackableFragment* StackableFragment = ItemComponent->GetItemManifestMutable().GetFragmentOfTypeMutable<FOldMIS_StackableFragment>())
	{
...
void UOldMIS_InventoryComponent::Server_AddStacksToItem_Implementation(AActor* ItemActor, int32 StackCount, int32 Remainder)
{
	if (!IsValid(ItemActor))
	{
	...
	UOldMIS_InventoryItem* Item = InventoryList.FindFirstItemByType(ItemType);
...
void UOldMIS_InventoryComponent::Server_DropItem_Implementation(UOldMIS_InventoryItem* Item, int32 StackCount)
{
	if (!IsValid(Item))
	{
...
void UOldMIS_InventoryComponent::SpawnDroppedItem(UOldMIS_InventoryItem* Item, int32 StackCount)
{
	...
	FOldMIS_ItemManifest& ItemManifest = Item->GetItemManifestMutable();
	if (FOldMIS_StackableFragment* StackableFragment = ItemManifest.GetFragmentOfTypeMutable<FOldMIS_StackableFragment>())
	{
...
void UOldMIS_InventoryComponent::Server_ConsumeItem_Implementation(UOldMIS_InventoryItem* Item)
{
	...
	if (FOldMIS_ConsumableFragment* ConsumableFragment = Item->GetItemManifestMutable().GetFragmentOfTypeMutable<FOldMIS_ConsumableFragment>())
	{
		ConsumableFragment->OnConsume(OwningController.Get());
	}
}

void UOldMIS_InventoryComponent::Server_EquipSlotClicked_Implementation(UOldMIS_InventoryItem* ItemToEquip, UOldMIS_InventoryItem* ItemToUnequip)
{
	DH_PRINT(EDH_Output::Both, 4.f, DHColors::Green,
...
void UOldMIS_InventoryComponent::Multicast_EquipSlotClicked_Implementation(UOldMIS_InventoryItem* ItemToEquip, UOldMIS_InventoryItem* ItemToUnequip)
{
	DH_PRINT(EDH_Output::Both, 4.f, DHColors::Green,
...
void UOldMIS_InventoryComponent::RequestEquipSlotClicked(UOldMIS_InventoryItem* ItemToEquip, UOldMIS_InventoryItem* ItemToUnequip)
{
	DH_PRINT(EDH_Output::Both, 4.f, DHColors::Green,
...
void UOldMIS_InventoryComponent::RequestDropItem(UOldMIS_InventoryItem* Item, int32 StackCount)
{
	if (!IsValid(Item)) return;
	Server_DropItem(Item, StackCount);
}

void UOldMIS_InventoryComponent::RequestConsumeItem(UOldMIS_InventoryItem* Item)
{
	if (!IsValid(Item)) return;
	Server_ConsumeItem(Item);
}

void UOldMIS_InventoryComponent::AddRepSubObj(UObject* SubObj)
{
...
TArray<UOldMIS_InventoryItem*> UOldMIS_InventoryComponent::GetAllItems()
{
	return InventoryList.GetAllItems();
}

UOldMIS_InventoryItem* UOldMIS_InventoryComponent::FindFirstItemByType(const FGameplayTag& ItemType)
{
	return InventoryList.FindFirstItemByType(ItemType);
}

bool UOldMIS_InventoryComponent::IsInventoryOpen() const
{
